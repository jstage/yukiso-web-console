...
.infor-card-con-hover{
   ...
   // position:relative;
    ...
    text-align: center;
    display: none;
    background-color:rgb(241, 161, 191);
    border-radius: 25px;
    padding: 10px 10px;
    // height:40px;
     margin-top: 80px;
    ...
    <Card :padding="0">
        <div class="infor-card-con">
            ...
            <div v-if="isShowTotalMessageCount" class='infor-card-con-hover'>
                <div><small>队列未被确认消息总量:</small>{{queueSize}}</div>       
            </div>
             <div v-if="isShowTotalDequeueCount" class='infor-card-con-hover'>
                <div><small>队列出队总数:</small>{{QdequeueCount}}</div>
                <div><small>主题出队总数:</small>{{TdequeueCount}}</div>
            </div>
            <div v-if="isShowTotalEnqueueCount" class='infor-card-con-hover'>
                <div><small>队列入队总数:</small>{{QenqueueCount}}</div>
                <div><small>主题入队总数:</small>{{TenqueueCount}}</div>       
            </div>
        </div>
    </Card>
</template>
