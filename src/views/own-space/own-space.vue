        ...
        <Card>
            ...
            <div>
                ...
                >
                    ...
                    <FormItem label="登录密码：">
                        <Button type="primary" style="width: 100px;" @click="showEditPassword">修改密码</Button>
                    </FormItem>
                    <div> 
  ...
  methods: {
      ...
      this.$refs["editPasswordForm"].validate(valid => {
        if (valid) {
          ...
          axios
            ...
            .post(
              "/validate",
              qs.stringify({ password: this.editPasswordForm.oldPass.trim() })
            )
            .then(
              () => {
                return axios
                  ...
                  .post(
                    "/change",
                    qs.stringify({ password: this.editPasswordForm.newPass.trim() })
                  );
              },
              error => {
    ...
    saveInfoAjax() {
      ...
      axios.create({
        ...
        .post("/change", qs.stringify({ username: this.userForm.name.trim() }))
        .then(
          () => {
            this.$Message.success("修改成功!");
